...
class FloatTensor extends Tensor {
    ...
    protected void multiplyRecursive(Tensor multiplier, Tensor result, int a, int b, int c, int z, int n, int[] d) {
        if (n < z - 1) {
            ...
            result.setFloatValue(multiplicationSum(multiplier, a, b, c, d, 0, new int[c]), d);
        }
    }

    private float multiplicationSum(Tensor multiplier, int a, int b, int c, int[] d, int n, int[] e) {
        if (n < c - 1) {
            float s = 0;
            ...
            return getFloatValue(rd1) * multiplier.getFloatValue(rd2);
        }
    }
